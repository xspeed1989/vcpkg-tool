...
#undef OUTDATED_V_NO_120
                                                ...
                                                MessageSink& msg_sink)
    {
        ...
        for (const Path& lib : libs)
        {
            ...
            switch (expect_release ? build_info.crt_linkage : build_info.debug_crt_linkage)
            {
                case LinkageType::DYNAMIC:
                    fail |= this_lib.has_static_debug;
                    fail |= this_lib.has_static_release;
    ...
    struct OutdatedDynamicCrtAndFileCompare
    {
        ...
        {
            std::string str_lhs;
            std::transform(lhs.outdated_crt.begin(),
                           lhs.outdated_crt.end(),
                           std::back_inserter(str_lhs),
                           tolower);
            str_lhs += lhs.file.native();
            std::string str_rhs;
            std::transform(rhs.outdated_crt.begin(),
                           rhs.outdated_crt.end(),
                                                         ...
                                                         MessageSink& msg_sink)
    {
        ...
        for (const PostBuildCheckDllData& dll_data : dlls_data)
        {
            for (const StringLiteral& outdated_crt : outdated_crts)
            {
                ...
                {
                    if (Strings::case_insensitive_ascii_contains(outdated_crt, "msvcrt.dll"))
                    {
                        whitelist_outdated_crt.insert({dll_data.path, "msvcrt.dll"});
                        continue;
                    }
                    if (Strings::case_insensitive_ascii_contains(outdated_crt, "msvcp60.dll"))
                    {
                        whitelist_outdated_crt.insert({dll_data.path, "msvcp60.dll"});
                        continue;
                    }
                    if (Strings::case_insensitive_ascii_contains(outdated_crt, outdated_crt))
                    {
        ...
        if (whitelist_outdated_crt.size())
            msg_sink.print(Color::success, "-- Detected system dynamic CRT in the following files:\n\n");
        for (const auto& btf : whitelist_outdated_crt)
        {
            msg_sink.print(Color::success, fmt::format("    {}:{}\n", btf.file, btf.outdated_crt));
        }
